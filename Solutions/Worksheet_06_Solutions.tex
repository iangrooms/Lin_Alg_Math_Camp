...
\item Prove the following: the iteration defined by $\vec{x}_{k+1}=\mathbf{B}\vec{x}_k$, where $\mathbf{B}\in\mathbb{R}^{n\times n}$ and $\vec{x}_k\in\mathbb{R}^n$,  converges to $\vec{0}$ for every initial condition $\vec{x}_0$ if and only if $\rho(\mathbf{B})<1$ where $\rho(\mathbf{B})$ is the spectral radius of $\mathbf{B}$.
...
Alternatively, you can use the fact that if $\rho(\mathbf{B})<1$ then there is an operator norm $\|\cdot\|$ such that $\rho(\mathbf{B})<\|\mathbf{B}\|<1$.
For this operator norm we have
\[\|\vec{x}_k\| = \|\mathbf{B}^k\vec{x}_0\|\le \|\mathbf{B}^k\|\|\vec{x}_0\|\le \|\mathbf{B}\|^k\|\vec{x}_0\|\to0\text{ as }k\to\infty.\]
Since we're dealing with a finite-dimensional space, the fact that the norm goes to zero implies that the vector goes to zero (in every norm).
